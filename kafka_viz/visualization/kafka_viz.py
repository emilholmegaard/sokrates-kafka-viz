...
class KafkaViz(BaseGenerator):
    ...
    def extract_kafka_communication(slef, services) -> dict:
    ...
    @staticmethod
    def generate_react_app(visualization_data, output_dir):
        """Generate a React application with the visualization."""
        os.makedirs(output_dir, exist_ok=True)

        # Save visualization data as JSON
        with open(os.path.join(output_dir, "visualization-data.json"), "w") as f:
    ...
    <body>
        ...
        <script src="visualization-data.json" type="text/javascript"></script>
...
def generate_output(self, data: dict, file_path: Path) -> None:
    try:
        # Parse the JSON file
        services = parse_kafka_data(data)

        # Extract Kafka communication data
        kafka_data = extract_kafka_communication(services)

        # Generate the visualization data
        visualization_data = generate_visualization_data(services, kafka_data)

        # Generate the React app
        generate_react_app(visualization_data, file_path)

        print("Kafka communication visualization generated successfully!")

        print(f"\n[green]Visualization generated at {file_path}")

    except Exception as e:
        print(f"[red]Error generating visualization: {e}")
