...
class JavaAnalyzer(BaseAnalyzer):
    ...
    def _add_topic(
        ...
        location = {"file": str(file_path), "line": str(line)}
    ...
    def _analyze_content(
        ...
        for match in re.finditer(r'@Value\s*\(\s*["\']?\$\{([^}]+)\}["\']?\)', content):
            ...
            if "kafka" in value.lower() and "topic" in value.lower():
                ...
                context = content[
                    max(0, match.start() - 200) : min(len(content), match.end() + 200)
                ]
                is_producer = any(
                    word in context.lower()
                    for word in ["producer", "send", "publish", "@sendto"]
                )
        ...
        for var_name in self.constant_map:
            ...
            for match in re.finditer(
                rf"(?:producer|template)\.(?:send|publish)\s*\(\s*{var_name}\b", content
            ):
                line = content[: match.start()].count("\n") + 1
            ...
            for match in re.finditer(
                rf"consumer\.subscribe\s*\([^)]*{var_name}\b", content
            ):
                line = content[: match.start()].count("\n") + 1
